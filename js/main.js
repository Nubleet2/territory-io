...
function _buildOverlay() {
    ...
    document.getElementById('start-btn').addEventListener('click', () => {
        ...
        if (_multiplayerMode && _serverUrl) _initMultiplayer(_serverUrl);
        else _init();
    });
    ...
    document.getElementById('btn-solo').addEventListener('click', () => {
        ...
        document.getElementById('btn-solo').classList.add('selected');
        document.getElementById('btn-multi').classList.remove('selected');
    });
    document.getElementById('btn-multi').addEventListener('click', () => {
        ...
        document.getElementById('btn-multi').classList.add('selected');
        document.getElementById('btn-solo').classList.remove('selected');
        _refreshServerList();
    });
}
...
function _buildShipCards() {
    ...
    Object.entries(SHIP_CLASSES).forEach(([id,cls]) => {
        ...
        const s=cls.stats;
        const bar=(v,max)=>{const pct=Math.round((v/max)*100);const col=v>=1.3?'#44ff88':v>=1.0?'#ffcc22':'#ff6644';return `<div style="display:inline-block;width:${pct}%;height:3px;background:${col};border-radius:1px;vertical-align:middle"></div>`;};
        card.innerHTML=`<div class="sname" style="color:${hex}">${cls.name.toUpperCase()}</div><div class="sdesc">${cls.desc}</div><div class="sstats">DMG &nbsp;${bar(s.damage,2)}<br>SPD &nbsp;${bar(s.speed,2)}<br>HULL ${bar(s.hull,2.5)}<br>SHLD ${bar(s.shield,2)}</div>`;
...
function _selectShipClass(id) {
    ...
    if(a){a.classList.add('selected');a.style.borderColor='#'+SHIP_CLASSES[id].color.toString(16).padStart(6,'0');}
}
...
function _buildServerBrowser() {
    ...
    sb.innerHTML = `
        ...
        <div style="display:flex;gap:8px;margin-bottom:14px">
            <input id="server-input" placeholder="hostname e.g. territory-io.up.railway.app"
                style="flex:1;background:rgba(0,10,30,0.8);border:1px solid rgba(255,255,255,0.15);
                color:#fff;padding:8px 12px;font-family:'Courier New';font-size:11px;border-radius:3px;outline:none;"/>
            <button id="btn-add-server" style="background:rgba(0,200,255,0.1);border:1px solid #0cf;
                color:#0cf;padding:8px 16px;cursor:pointer;font-family:'Courier New';font-size:11px;border-radius:3px;">ADD</button>
        </div>
        ...
        <div style="display:flex;gap:8px;justify-content:center">
            <button id="btn-refresh" style="background:transparent;border:1px solid rgba(255,255,255,0.2);
                color:rgba(255,255,255,0.5);padding:8px 20px;cursor:pointer;font-family:'Courier New';font-size:10px;border-radius:3px;">↺ REFRESH</button>
        </div>`;
...
function _refreshServerList() {
    ...
    list.innerHTML = _savedServers.length
        ? '<div style="font-size:10px;color:rgba(255,255,255,0.25);letter-spacing:1px">Pinging...</div>'
        : '<div style="font-size:10px;color:rgba(255,255,255,0.25)">No servers saved.</div>';
...
function _renderServerCard(list, host, info) {
    const old=document.getElementById('srv-'+host.replace(/[:.]/g,'_')); if(old) old.remove();
    const card=document.createElement('div');
    card.id='srv-'+host.replace(/[:.]/g,'_');
    card.style.cssText='background:rgba(0,10,30,0.8);border:1px solid rgba(255,255,255,0.12);padding:10px 14px;border-radius:3px;display:flex;align-items:center;gap:12px;';
    if(!info){
        card.innerHTML=`<div style="flex:1"><div style="font-size:11px;color:rgba(255,255,255,0.4)">${host}</div><div style="font-size:9px;color:#ff4444;margin-top:2px">Offline</div></div><button onclick="_removeServer('${host}')" style="background:transparent;border:1px solid rgba(255,50,50,0.3);color:rgba(255,80,80,0.6);padding:4px 10px;cursor:pointer;font-family:'Courier New';font-size:9px;border-radius:2px;">✕</button>`;
    } else {
        const terr=Object.entries(info.territoryPct||{}).map(([f,pct])=>`<span style="color:${FACTIONS[f]?.colorHex||'#fff'}">${FACTIONS[f]?.name.split(' ')[0]||f} ${pct}%</span>`).join(' · ');
        const mins=Math.floor(info.gameTime/60),secs=info.gameTime%60;
        const pc=info.ping<50?'#44ff88':info.ping<120?'#ffcc22':'#ff6644';
        card.innerHTML=`<div style="flex:1"><div style="font-size:12px;color:#fff;margin-bottom:3px">${info.name||host}</div><div style="font-size:9px;color:rgba(255,255,255,0.4);margin-bottom:3px">${host}</div><div style="font-size:9px;color:rgba(255,255,255,0.35)">${terr||'—'}</div></div><div style="text-align:right;min-width:90px"><div style="font-size:10px;color:#0cf">${info.playerCount}/${info.maxPlayers}</div><div style="font-size:9px;color:rgba(255,255,255,0.4);margin-top:2px">${mins}:${secs.toString().padStart(2,'0')}</div><div style="font-size:9px;color:${pc};margin-top:2px">${info.ping}ms</div></div><button onclick="_joinServer('${host}')" style="background:rgba(0,200,255,0.1);border:1px solid #0cf;color:#0cf;padding:8px 14px;cursor:pointer;font-family:'Courier New';font-size:11px;border-radius:3px;">JOIN</button>`;
    }
...
function _joinServer(host) {
    _serverUrl=host; _multiplayerMode=true;
    document.getElementById('server-browser').style.display='none';
    document.getElementById('deploy-section').style.display='';
...
// ── INIT ──────────────────────────────────────────────────────────────────
function _init() {
    Render.init(); Effects.init(); Audio.init();
    GAME.worldSeed = 12345; // fixed seed for singleplayer
...
function _initMultiplayer(host) {
    ...
    // Everyone uses a shared world seed — host picks it, clients match
    // For now use a fixed seed; host could broadcast their seed on connect
...
function _loop(now) {
    ...
    GAME.clock.last=now;
    Player.update(dt); World.update(dt); Buildings.update(dt);
    AI.update(dt); Combat.update(dt); Effects.update(dt);
    UI.update(dt); Render.render();
}
